...
function ScoreInputs({ criteria, scores, autoFlags, onChange, onScoreChange, onSuggest, canSuggest }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div style={{
        display: 'flex', alignItems: 'flex-start', gap: 8, padding: '10px 12px',
        background: 'var(--bg-dark)', border: '1px solid var(--accent-dark)',
        borderRadius: 6, fontSize: '0.78rem', color: 'var(--text-dim)', marginBottom: 4
      }}>
        <Info size={15} style={{ flexShrink: 0, marginTop: 1, color: 'var(--accent)' }} />
        <span>
          Scores are <b style={{ color: 'var(--accent)' }}>auto-suggested</b> from the idea's name, description
          and category. Drag any slider to override it manually — once you move a slider it switches to <b>Manual</b>.
        </span>
      </div>
      ...
      {criteria.map(c => {
        const isAuto = autoFlags[c.id] !== false; // undefined/true = auto, false = manually overridden
        return (
          <div key={c.id}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
              <label className="label" style={{ marginBottom: 0, display: 'flex', alignItems: 'center', gap: 7 }}>
                ...
                <span style={{
                  ...
                  border: `1px solid ${isAuto ? 'var(--accent-secondary)' : 'var(--border)'}`,
                  opacity: 0.85,
                }}>
                  {isAuto ? 'Auto' : 'Manual'}
                </span>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                {!isAuto && (
                  <button
                    title="Reset this criterion to its auto-suggested value"
                    onClick={() => onSuggest(c.id)}
...
export default function IdeaModal({ idea, criteria, onSave, onClose }) {
  ...
  // For a brand-new idea, attempt an immediate suggestion if there's already
  // some text (e.g. re-opening a draft); otherwise fall back to flat 5s until
  // the user types something and/or clicks Suggest Scores.
  const initialScores = idea?.scores || defaultScores;
  // autoFlags: true = value currently reflects the auto-suggestion engine,
  // false = user manually dragged this slider (so re-suggesting won't clobber it).
  ...
  // Re-run suggestions. If `onlyCriterionId` is passed, only that one criterion
  // is reset to its suggested value (used by the per-row "reset to auto" button).
  // Otherwise, every criterion still flagged Auto gets refreshed — manually
  // overridden ones are left untouched so the user's edits aren't lost.
  const runSuggest = (onlyCriterionId) => {
    ...
    setForm(f => {
      ...
      criteria.forEach(c => {
        if (autoFlags[c.id] !== false) nextScores[c.id] = suggested[c.id];
      });
  ...
  const handleScoreChange = (criterionId, value) => {
    ...
    setAutoFlags(f => ({ ...f, [criterionId]: false })); // user took manual control
  };

  // Auto pre-fill as the user types name/description on a NEW idea (debounced
  // implicitly by only firing on field change, which is cheap here since the
  // scoring engine is just keyword matching, not a network call).
